        ...
        while (true) {
            ...
            await responsePromise.then((data) => {
                ...
                if (!this.recover(err, call, request, requestSendStarted)) {
                    if (!!request.client && !!request.client.eventListeners) {
                        request.client.eventListeners.callFailed(call, err);
                    }
                    // 如果没有必要重试，拦截器会直接抛出异常
            ...
            if (!followUp) { //如果构建的request为空，不再进行请求，直接返回response
                if (error) {
                    if (!!request.client && !!request.client.eventListeners) {
                        request.client.eventListeners.callFailed(call, error)
                    }
                    return Promise.reject(error)
                }
                if (!!request.client && !!request.client.eventListeners) {
                    request.client.eventListeners.callEnd(call);
                }
                return response
            }
